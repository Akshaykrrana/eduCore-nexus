...
@app.route('/Akshay')
def Akshay():
    return render_template('Akshay.html')
@app.route('/Ak_Cont')
def Ak_cont():
    return render_template('Ak_Cont.html')
...
def edit(id):
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"UPDATE `Student` SET `rollno`='{rollno}',`sname`='{sname}',`sem`='{sem}',`gender`='{gender}',`branch`='{branch}',`email`='{email}',`number`='{num}',`address`='{address}' WHERE id='{posts.id}'")
